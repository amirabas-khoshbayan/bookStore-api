...
import (
	...
	"github.com/amiremperor/bookStore-api/internal/contract"
...
func (m *messageBundle) Translate(lang contract.Lang,key string) string  {
	message , err := m.getLocalized(string(lang)).Localize(&translator.LocalizeConfig{
		MessageID: key,
	})
	if err != nil {
		return key
...
func (m *messageBundle) TranslateEn(key string) string {
	message , err := m.getLocalized("en").Localize(&translator.LocalizeConfig{
		MessageID: key,
	})
	if err != nil {
		return key
